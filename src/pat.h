 ...
 * seed in '_randSeed'.
 */
static inline UINT32 genRandSeed(const prefixMatching::BTDnaString& qry,
                                   const prefixMatching::BTString& qual,
                                   const prefixMatching::BTString& name,
                                   UINT32 seed)
{
	// Calculate a per-read random seed based on a combination of
	// the read data (incl. sequence, name, quals) and the global
	// seed
    UINT32 rseed = (seed + 101) * 59 * 61 * 67 * 71 * 73 * 79 * 83;
...
struct PatternParams {
    ...
    PatternParams(const char* coded){
        deli = kmers_Delimitor::getInstance();
        try{
            if(!deli)
                throw invalid_argument("can not initiate kmers_deliminator!");
            this->decode(coded);
            this->deli = kmers_Delimitor::getInstance();
    ...
    UINT32 seed;        // pseudo-random seed
    ...
    UINT32 skip;        // skip the first 'skip' patterns
	bool isInMem;
    kmers_Delimitor *deli;
	const char* encode(){
       // string coded;
        char *buf = new char[30];
        sprintf(buf,"%c%d%c%d%c%d%c%d%c%d%c%d%c%d%c%d%c%d%c%d%c%d%c%d%c",
                                deli->insideDeli(),this->format,
                                deli->insideDeli(),this->fileParallel,
                                deli->insideDeli(),this->seed,
                                deli->insideDeli(),this->useSpinlock,
                                deli->insideDeli(),this->solexa64,
                                deli->insideDeli(),this->phred64,
                                deli->insideDeli(),this->intQuals,
                                deli->insideDeli(),this->sampleFreq,
                                deli->insideDeli(),this->sampleLen,
                                deli->insideDeli(),this->skip,
                                deli->insideDeli(),this->fuzzy,
                                deli->insideDeli(),this->isInMem,

                                deli->outsideDeli());

        return buf;
	}
	void decode(const char* coded){
        string tmp;
        size_t pos = 0;
        size_t len = strlen(coded);
        assert_gt(len,0);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),this->format);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),(int&)this->fileParallel);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),(int&)this->seed);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),(int&)this->useSpinlock);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),(int&)this->solexa64);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),(int&)this->phred64);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),(int&)this->intQuals);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),this->sampleFreq);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),this->sampleLen);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),(int&)this->skip);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),(int&)this->fuzzy);

        getCodedParam(coded,pos,tmp);
        getParams(tmp.c_str(),(int&)this->isInMem);

        tmp.clear();

	}

	void getParams(const char *p,int& param){
        if(NULL == p){
	...
	/**
      ...
      */
    void getCodedParam(const char*p,size_t &pos,string& tmp){
        if(NULL == p){
            cerr<<"invalid coded format of pattern source parameter!"<<endl;
            cerr.flush();
            throw 1;
        ...
        size_t len = strlen(p);
        assert( (p[pos]==deli->insideDeli()) | (p[pos]==deli->outsideDeli()) );
        pos++;
        tmp.clear();
        while(p[pos] != deli->insideDeli() && p[pos] != deli->outsideDeli() && pos < len ){
            tmp += p[pos];
            pos++;
        }
        assert(  ((p[pos]==deli->insideDeli()) | (p[pos]==deli->outsideDeli())) | (pos == len) );
    }
};
...
public:
    PairedPatternSource(const PatternParams& p,bool lock = false) : mutex_m(), seed_(p.seed),_dolocking(lock) {}
	virtual ~PairedPatternSource() { }

	virtual void addWrapper() = 0;
	virtual void reset() = 0;
    inline bool loadReads(vector<std::shared_ptr<Read> >& result,size_t& num){
        ...
        while((total < num) && !done){


            TReadId rdid;
            TReadId endid;
            shared_ptr<Read> buf1(new Read());
            shared_ptr<Read> buf2(new Read());
            do{
                this->nextReadPair(*buf1,*buf2,rdid,endid,success,done,pair,fixName);
            //reach the end of the file
            }while(!success && !done);
            //dump the reads into fq format string for engine use
            //this is a unpaired read
            if(success){
                //buf1.rdid = ((_totalReads << _offset) | _suffix); //add suffix to the read id
...
protected:
    void clear(vector<string>& result){
...
protected:
    ...
    volatile UINT32 cur_; // current element in parallel srca_, srcb_ vectors
...
public:
    ...
    virtual EList<PatternSourcePerThread*>* create(UINT32 n) const = 0;
...
public:
	 ...
	 */
    virtual EList<PatternSourcePerThread*>* create(UINT32 n) const {
...
public:
	BufferedFilePatternSource(
		...
		isInMem_(p.isInMem)
	{
        //this->isInMem_ = p.isInMem;
		assert_gt(infiles.size(), 0);
		/*for(unsigned i=0; i< infiles.size(); i++){
            try{
                this->infiles_.push_back(infiles[i]);
            }catch(bad_alloc& e){
                cerr<<"Can not allocate the size of "<< infiles[i].length()<<" byte to engine!"<<endl;
                exit(0);
            }
		}*/
...
protected:
        ...
        if(this->isInMem_){
            ...
            while(filecur_ < infiles_.size()) {
                ...
                if(infiles_[filecur_] == "-") {
                    in = stdin;
                } else if((in = fmemopen((char *)infiles_[filecur_].c_str(), infiles_[filecur_].length(), "rb")) == NULL) {
                    if(!errs_[filecur_]) {
                        cerr << "Warning: Could not open the"<< filecur_<< " read file \""  << "\" for reading; skipping..." << endl;
                        errs_[filecur_] = true;
                    }
                    filecur_++;
                    continue;
	...
	const EList<string>& infiles_;  // filenames for read files
...
private:
    ...
    UINT64 subReadCnt_;/// number to subtract from readCnt_ to get
